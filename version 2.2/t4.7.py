...
class Device:
    def __init__(self, device_id, qualities):
        self.id = device_id  # Unique ID for the device
...
def objective2(X):
    return X[:, 1] ** 2  # Maximize the second quality

def objective3(X):
    # return np.sin(X[:, 2])  # Oscillatory objective
    return X[:, 2] ** 2  # Maximize the third quality

def objective4(X):
    # return X[:, 3]  # Maximize the fourth quality
    return X[:, 3] ** 2  # Maximize the fourth quality

def objective5(X):
    # return X[:, 4] + X[:, 5]  # Sum of fifth and sixth qualities
    return X[:, 4] ** 2  # Maximize the fifth quality

def objective6(X):
    return -np.sum(X, axis=1)  # Minimize the sum of all qualities
...
np.random.seed(42)
devices = [Device(i + 1, [np.random.randint(1, 65)] + list(np.random.rand(5))) for i in range(100)]
...
for round_num in range(num_rounds):
    ...
    # Update device qualities dynamically (simulate real-world changes)
    for device in devices:
        device.qualities[1:] += np.random.uniform(-0.1, 0.1, size=5)  # Randomly adjust qualities
        device.qualities[1:] = np.clip(device.qualities[1:], 0, 1)  # Ensure qualities stay within bounds
    ...
    pareto_indices = pareto_indices[:min(len(pareto_indices), selected_device_count)]  # Limit to selected_device_count
    selected_devices = [devices[i] for i in pareto_indices]
    non_selected_devices = [devices[i] for i in range(len(devices)) if i not in pareto_indices]
    
    # Print Pareto front details
    print("\nPareto Front Devices and Their Objective Values:")
    for i in pareto_indices:
        print(f"Device {devices[i].id}: Qualities = {devices[i].qualities}, Objectives = {res.F[i]}")

    print(f"\nSelected Devices: {[device.id for device in selected_devices]}")
    print(f"Non-Selected Devices: {[device.id for device in non_selected_devices]}")
    ...
    for i, device in enumerate(selected_devices):
        ...
        print(f"Device {device.id} trained on its local dataset partition.")
...
main_model = Device(0, [0]*6)  # Create a dummy device for final evaluation
