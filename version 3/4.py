# Re-import necessary packages and recreate the solution due to environment reset
import random
import numpy as np
import pandas as pd
from deap import base, creator, tools, algorithms

# Reload the device data from the original file path
...
# Adding device ids to the Pareto front output
device_ids = df["id"].values  # Getting device ids from the CSV data

# Collecting Pareto front results with ids
pareto_front_with_ids = []
for front in pareto_fronts:
    for ind in front:
        device_index = population.index(ind)
        device_id = device_ids[device_index]
        pareto_front_with_ids.append([device_id, ind.fitness.values[0], ind.fitness.values[1]])

# Convert to DataFrame for easy viewing and save to CSV
pareto_df_with_ids = pd.DataFrame(pareto_front_with_ids, columns=["Device ID", "Energy Consumption", "Battery Level"])
pareto_csv_filename = "data/pareto_fronts_with_ids.csv"
pareto_df_with_ids.to_csv(pareto_csv_filename, index=False)
