...
toolbox.register("attr_bool", lambda: random.choice([0, 1]))  # Ensure true randomness
...
best_solution = max(pareto_front, key=lambda ind: sum(ind))  # Pick solution with most devices selected

selected_devices = [devices[i] for i in range(NUM_DEVICES) if best_solution[i] == 1]
