...
for i in range(problem.n_obj):
    for j in range(i + 1, problem.n_obj):
        plt.subplot(problem.n_obj - 1, problem.n_obj - 1, i * (problem.n_obj - 1) + j)

        # Plot all solutions
        plt.scatter(res.F[:, i], res.F[:, j], color="red", alpha=0.5, label="All Solutions" if i == 0 and j == 1 else "")

        # Plot Pareto front
        plt.scatter(pareto_front[:, i], pareto_front[:, j], color="blue", s=30, label="Pareto Front" if i == 0 and j == 1 else "")
